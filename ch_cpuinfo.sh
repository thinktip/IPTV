...
GATHER_FN () {
    # 提取完整的 AMD CPU 名称
    cpu_fullname=$(grep -m1 'model name' /proc/cpuinfo | cut -d ':' -f2- | sed 's/^ *AMD *//')

    if echo "$cpu_fullname" | grep -qi "AMD"; then
        cpu_vendor="AMD"
    else
    ...
    # 分离 family 和 series
    cpu_family=$(echo "$cpu_fullname" | awk '{print $1, $2}') # 例：Ryzen 3
    cpu_series=$(echo "$cpu_fullname" | sed "s/^$cpu_family //")

    # 构造跳转链接
    cpu_search_url="https://www.amd.com/en/search?keyword=$(echo "$cpu_family $cpu_series" | sed 's/ /%20/g')"
    cpu_detail="<a href='${cpu_search_url}' target=_blank>detail</a>"

    # 核心、线程信息提取
    PICNT=$(grep -c '^physical id' /proc/cpuinfo)
    [ "$PICNT" -eq 0 ] && PICNT=1
    CCCNT=$(grep -m1 '^cpu cores' /proc/cpuinfo | awk '{print $4}')
    [ -z "$CCCNT" ] && CCCNT=$(nproc)
    THCNT=$(grep -c '^processor' /proc/cpuinfo)
    ...
    TCCNT_STR="${TCCNT} Core"
    [ "$TCCNT" -gt 1 ] && TCCNT_STR="${TCCNT} Cores"
    ...
    cpu_cores="$TCCNT_STR ($TPCNT$PCCNT| $TTCNT)"
}
